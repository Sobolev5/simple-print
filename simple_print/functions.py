...
def sprint(*args):
    ...
    if re.compile(r'sprint\(\)').match(code):          
        ...
        if variables:              
            print_array = []
            variables_list_len = len(variables) - 1
            for i, variable in enumerate(variables):

                if variables_list_len == i and variables_list_len > 0 and variable.strip('"').strip("'") in ['grey', 'red', 'green', 'yellow', 'blue', 'magenta', 'cyan', 'white']:
    ...
    if print_string:        
        ...
        print(f'{args} :: line {lineno}', color, attrs=['bold'])
...
def sprint_f(*args):
    ...
    if print_string:        
        ...
        cprint(f'{args} :: line {lineno} :: {filename}', color, attrs=['bold'])
